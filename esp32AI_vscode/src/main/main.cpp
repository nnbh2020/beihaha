...
// 定义是否启用屏幕功能
#define ENABLE_SCREEN 0 // 设置为1以启用屏幕功能
...
String APPID = "d140859a";                             // App ID,必填
String APISecret = "ZDcyZjZhNGMzOWNmODJmYTYwYzkzNjIx"; // API Secret，必填
String APIKey = "0a470d3be28eea6b6a5d3f816f27d716";    // API Key，必填
String appId1 = APPID;
...
void voicePlay()
{
    ...
    if ((audio2.isplaying == 0) && (Answer != "" || subindex < subAnswers.size()))
    {
        if (subindex < subAnswers.size())
        {
            ...
            {
                /*                 // 清空屏幕
                                screenHandler.tft.fillScreen(TFT_WHITE);
                                // 显示剩余的文字
    ...
    else if (audio2.isplaying == 0 && musicplay == 1) // 处理连续播放音乐逻辑
    {
        ...
        /*         screenHandler.tft.fillRect(0, cursorY, width, 50, TFT_WHITE);
...
{
    /*     screenHandler.tft.fillScreen(TFT_WHITE);
        screenHandler.tft.setCursor(0, 0);
        screenHandler.tft.print("assistant: "); */
    audio2.connecttospeech(Answer.c_str(), "zh");
...
void processCommand(uint8_t command)
{
    ...
    case 0x01:
        ...
        startPlay = true;
        chouxiang = 1;
        // screenHandler.tft.pushImage(0, 0, width, height, guichu[0]);   // 丁真
    ...
    case 0x02:
        ...
        startPlay = true;
        chouxiang = 3;
        // screenHandler.tft.pushImage(0, 0, width, height, guichu[2]);   // 牢大
        awake_flag = 1; // 命令词识别成功后，标识为唤醒状态
        break;
    case 0x03:
        ...
        startPlay = true;
        chouxiang = 2;
        // screenHandler.tft.pushImage(0, 0, width, height, guichu[1]);   // 坤坤
        awake_flag = 1; // 命令词识别成功后，标识为唤醒状态
        break;
    case 0x04:
...
/* bool tft_output(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t* bitmap)
{
  ...
  screenHandler.tft.pushImage(x, y, w, h, bitmap);
...
void setup()
{
    // 初始化串口通信，波特率为115200
    Serial.begin(115200);
    Serial.println("基础测试：串口已启动");
    Serial.flush(); // 确保数据发送完成
    printf("printf基础测试：串口已启动");
#ifdef ASRPRO
    // 初始化 UART2
    ...
    screenHandler.init();
    screenHandler.displayMessage("屏幕初始化完成！");
    ...
    /*     // 初始化屏幕
        ...
        screenHandler.tft.fillScreen(TFT_WHITE);   // 设置屏幕背景为白色
        ...
        // 显示文字
        screenHandler.u8g2.setCursor(0, 11);
        screenHandler.u8g2.print("已开机！");
    ...
    preferences.begin("llm_store");
    // 连接网络
    /*     screenHandler.u8g2.setCursor(0, u8g2.getCursorY() + 12);
        screenHandler.u8g2.print("正在连接网络······"); */

    // 检查是否已经保存了WiFi信息
    int numNetworks = preferences.getInt("numNetworks", 0);
    if (numNetworks == 0)
    {
        // 如果没有保存WiFi信息，则使用默认的WiFi账号和密码
        preferences.putInt("numNetworks", 1);
        preferences.putString("ssid0", "ChinaNet-t8XU");
        preferences.putString("password0", "88888888");
        Serial.println("使用默认的WiFi账号和密码");
    }
    ...
    if (result == 1)
    {
        ...
        /*         // 清空屏幕，在屏幕上输出提示信息
                screenHandler.tft.fillScreen(TFT_WHITE);
                screenHandler.u8g2.setCursor(0, 11);
                screenHandler.u8g2.print("网络连接成功！");
...
void loop()
{
    ...
    if (currentMillis - previousMillis >= interval)
    {
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
            ...
            printf("Free Heap:%s", String(ESP.getFreeHeap()));
        ...
        // 如果音频播放结束，可以在这里添加重新播放的逻辑
        if (!audio2.isplaying && startPlay)
        {
            printf("音频播放结束，重新播放电台");
            audio2.connecttohost("http://lhttp.qingting.fm/live/4915/64k.mp3");
        }
    }
}
void loop2()
{
    ...
    if (millis() - endTime > 2000 && audio2.isplaying == 0 && ((shuaxin == 1 && chouxiang == 0) || (chouxiang != 0 && startPlay == false)))
    {
        /*             // 清空屏幕，在屏幕上输出提示信息
                    screenHandler.tft.fillScreen(TFT_WHITE);
                    screenHandler.u8g2.setCursor(0, 0);
...
#if ENABLE_SCREEN
        screenHandler.screenHandler.tft.pushImage(0, 90, width, 60, image0[i]); // 用于壁纸显示的代码
#endif
...
void displayWrappedText(const string &text1, int x, int y, int maxWidth)
{
    ...
    while (start < num)
    {
        screenHandler.u8g2.setCursor(cursorX, cursorY);
        int wid = 0;
        int numBytes = 0;
        ...
        {
            screenHandler.u8g2.print(text1.substr(start, numBytes).c_str());
            cursorY += lineHeight;
            cursorX = 0;
            start += numBytes;
        }
...
void getText(String role, String content)
{
    ...
    /*     screenHandler.tft.print(role);
        screenHandler.tft.print(": ");
        displayWrappedText(content.c_str(), tft.getCursorX(), tft.getCursorY() + 11, width);
        screenHandler.tft.setCursor(0, u8g2.getCursorY() + 2); */
}
...
#if ENABLE_SCREEN                             // 显示提取的句子
        screenHandler.tft.setCursor(54, 152); // 在屏幕上显示对话轮次
        screenHandler.tft.print(loopcount);
#endif
...
void VolumeSet()
{
    ...
    if ((askquestion.indexOf("显示") > -1 && askquestion.indexOf("音") > -1) || (askquestion.indexOf("音") > -1 && askquestion.indexOf("多") > -1))
    {
        ...
        /*         if (chouxiang != 0)
                {
                    screenHandler.tft.pushImage(0, 0, width, height, guichu[chouxiang - 1]);
                }
                else
                    screenHandler.tft.fillRect(66, 152, 62, 7, TFT_WHITE);    // 在屏幕上显示音量
                screenHandler.tft.setCursor(66, 152);
                screenHandler.tft.print("volume:");
                screenHandler.tft.print(volume); */
    }
...
void onEventsCallback1(WebsocketsEvent event, String data)
{
    ...
    if (event == WebsocketsEvent::ConnectionOpened)
    {
        ...
        if (conflag == 1)
        {
            ...
            screenHandler.tft.fillScreen(TFT_WHITE);
            screenHandler.u8g2.setCursor(0, 11);
            screenHandler.u8g2.print("连续对话中，请说话！");
        }
        else
        {
            screenHandler.u8g2.setCursor(0, 159);
            screenHandler.u8g2.print("请说话！");
        }
#else
        if (await_flag == 1)
        {
...
void processAskquestion()
{
    ...
    else if (audio2.isplaying == 1 && askquestion.indexOf("暂停") > -1)
    {
        ...
        if (chouxiang != 0)
        {
            ...
            screenHandler.tft.fillRect(0, 148, 50, 12, TFT_WHITE); // 清空左下角的“请说话！”提示
#endif
...
#if ENABLE_SCREEN
        screenHandler.u8g2.setCursor(0, u8g2.getCursorY() + 12);
        screenHandler.u8g2.print("无任何wifi存储信息！");
...
// 在屏幕上显示连接失败信息
#if ENABLE_SCREEN
                    screenHandler.u8g2.setCursor(u8g2.getCursorX() + 6, u8g2.getCursorY());
                    screenHandler.u8g2.print("Failed!");
#endif
                    break;
                }
            ...
            if (WiFi.status() == WL_CONNECTED)
            {
                ...
                // 在屏幕上显示连接成功信息
                screenHandler.u8g2.setCursor(u8g2.getCursorX() + 6, u8g2.getCursorY());
                screenHandler.u8g2.print("Connected!");
#endif
                preferences.end();
                return 1;
            }
    ...
    // 在屏幕上输出提示信息
    screenHandler.u8g2.setCursor(0, 11);
    screenHandler.u8g2.print("网络连接失败！请检查");
    screenHandler.u8g2.setCursor(0, u8g2.getCursorY() + 12);
    screenHandler.u8g2.print("网络设备，确认可用后");
    screenHandler.u8g2.setCursor(0, u8g2.getCursorY() + 12);
    screenHandler.u8g2.print("重启设备以建立连接！");
